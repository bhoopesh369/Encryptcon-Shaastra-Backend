		...
		proofRequests["General-Identity"] = {
			...
			requested_attributes: {
				attr1_referent: {
					name: "name",
					restrictions: [
						{ cred_def_id: await indy.did.getGovIdCredDefId() },
					],
				},
			},
			requested_predicates: {},
		};
		let transcriptCredDef = await indy.issuer.getCredDefByTag(
			"MyTranscript"
		);
...
exports.prepareRequest = async function (message) {
	...
	try {
		...
		return {
			...
			type: message.type,
			message:
				"Error: The target user doesnt have the requested credential in their wallet.",
		};
	}
...
exports.acceptRequest = async function (messageId) {
	...
	let [schemas, credDefs, revocStates] =
...
exports.validateAndStoreProof = async function (message) {
	...
	if (proofRequest) {
		let [schemas, credDefs, revRegDefs, revRegs] =
			await indy.pool.verifierGetEntitiesFromLedger(proof.identifiers);
		delete proof.nonce;
		if (
			true ||
