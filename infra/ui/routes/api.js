...
router.post("/send_message", auth.isLoggedIn, async function (req, res) {
	...
	res.send(await indy.crypto.sendAnonCryptedMessage(req.body.did, message));
...
router.post(
	...
	auth.isLoggedIn,
	async function (req, res) {
		res.send(indy.store.messages.deleteMessage(req.body.messageId));
		// res.redirect("/");
	}
);
...
router.post("/proofs/accept", auth.isLoggedIn, async function (req, res) {
	res.send(await indy.proofs.acceptRequest(req.body.messageId));
	// res.redirect("/#messages");
});
